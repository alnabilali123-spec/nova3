...
android {
    ...
    buildTypes {
        ...
        release {
            ...
            // Sign with the keystore provided via env vars (decoded by the GitHub Actions workflow)
            // or fall back to the debug key for local builds.
    ...
    signingConfigs {
        create("release") {
            // Only set when KEYSTORE_BASE64 env var is present (i.e. in CI).
            val base64 = System.getenv("KEYSTORE_BASE64")
            if (!base64.isNullOrBlank()) {
                val keystoreFile = file("novatube.keystore")
